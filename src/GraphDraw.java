...
public class GraphDraw extends JFrame {
        ...
        private void checkPosition(int i){
            for(Node e: nodes){
                if(nodes.get(i) != e)
                    if (nodes.get(i).x == e.x && nodes.get(i).y == e.y){
                        nodes.get(i).x += 30;
                    }
            }
        }

        private void drawNode(int i, int size, int x, int y){
            int leftSon = 2*i+1;
            int rightSon = 2*i+2;

            if(i < size){
                addNode(Integer.toString(i), x, y);
                drawNode(leftSon, size, x-(x/4), y+30);
                drawNode(rightSon, size, x+(x/4), y+30);
            }
        }

        public void drawfor(Graph g){
            int width = 800;
            int height = 600;
            int X = width/2;
            int Y = 50;
