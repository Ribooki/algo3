...
public class Grid {
    ...
    public Grid(int width, int height) {
        this.width = width;
        this.height = height;
        maxVertex = width * height;
        graph = new Graph(maxVertex);
        for (int i = 0; i < width; i++) {
            for (int j = 0; j < height; j++) {
                if (i < width - 1)
                    graph.addEdge(vertexOfCoordinate(i,j), vertexOfCoordinate(i+1,j), 0.0);
                if (j < height - 1)
                    graph.addEdge(vertexOfCoordinate(i,j), vertexOfCoordinate(i,j+1), 0.0);
            }
        }
